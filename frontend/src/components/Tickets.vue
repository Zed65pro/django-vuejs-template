  ...
  <div class="card m-5 p-5 d-flex flex-column">
    ...
    <h4 class="card-header-hr mt-1"><i class="fa fa-bookmark"></i>
      ...
      <span class="float-right" id="navigation-wrapper">
                            <button class="btn btn-dark btn-sm mr-1 move-page" v-if="previous"
            ...
            <div class="timeline-card card border-grey border-lighten-2 mr-3" style="background: #F3F4F9">
              <div class="card-header" style="background: #F3F4F9">
                <h4 class="card-title"><span
                    class="text-info">{{ ticket.type.type }} - {{
                    ticket.subject.subject
                  }} - {{ ticket.status ? 'Active' : 'Closed' }}</span>
                </h4>
                ...
                <div class="heading-elements">
                                <span class="card-subtitle text-muted d-inline-block">
                                    ...
                                    <span class="font-small-3 d-block"
                                          v-if="ticket.transfered_on">Transfered on {{ ticket.transfered_on }}</span>
                                </span>
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    fetchFormTypes: async function () {
      try {
        const response = await axios.get(this.loadTicketTypes);
    ...
    fetchFormSubjects: async function () {
      try {
        const response = await axios.get(this.loadTicketSubjects);
    ...
    fetchFormContent: async function () {
      ...
      try {
        ...
        const response = await axios.get(this.loadTicketTemplate.replace('0000', this.selectedType.id).replace('1111', this.selectedSubject.id));
        const formData = response.data['template']
        console.log(formData);
        if (formData) {
          let formRenderOptions = {formData};
          this.renderedForm = $('#rendered-form').formRender(formRenderOptions);
        } else {
    ...
    renderTicketsContent() {
      ...
      for (const ticket of this.tickets) {
        ...
        this.$nextTick(() => {
          try {
            let formRenderOptions = {formData: ticket.content};
            const x = $(`#ticket-content-${ticket.id}`).formRender(formRenderOptions);
            console.log(x)
          } catch (error) {
    ...
    fetchTickets: async function (url = null) {
      try {
        ...
        const response = await axios.get(url ? url : this.ticketsListCreateApiUrl);
    ...
    handleReplyClick(event, ticketId) {
      ...
      axios.post(this.ticketReplyCreateApiUrl.replace('0000', ticketId), {'content': content})
          .then(response => {
    ...
    addTicket: async function () {
      ...
      try {
        const response = await axios.post(this.ticketsCreateApiUrl, newTicket);
    ...
    closeTicket: async function (ticketId) {
      try {
        const response = await axios.patch(this.closeTicketUrl.replace('0000', ticketId));
